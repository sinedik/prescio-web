...
import EdgeCard from '../components/markets/EdgeCard'
...
const getFeed = api.getFeed as (params?: Record<string, string>) => Promise<any>
// eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const load = useCallback(async (isPolling = false, forceRefresh = false) => {
    ...
    try {
      if (viewMode === 'markets') {
        ...
        const items: MarketOpportunity[] = result?.items ?? []
        const s: boolean = result?.scanning ?? false
