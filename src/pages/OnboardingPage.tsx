...
export default function OnboardingPage() {
  ...
  async function finish() {
    ...
    await supabase
      .from('profiles')
      .update({ onboarding_done: true, interests: selected })
      .eq('id', user.id)
    await refreshProfile()
    navigate('/markets', { replace: true })
  }
...
function Step4({
  selected, onToggle, onFinish, saving,
}: {
  selected: string[]; onToggle: (id: string) => void
  onFinish: () => void; saving: boolean
}) {
  return (
    <div className="w-full animate-fade-in">
        ...
        {INTERESTS.map((interest) => {
          ...
          return (
            <button
              ...
              className={`py-3 px-4 rounded-xl border text-sm font-mono font-medium transition-all ${
                isActive
                  ? 'bg-accent/10 border-accent text-accent'
                  : 'bg-bg-surface border-bg-border text-text-secondary hover:border-text-muted'
              }`}
